...
import { AbstractUserLocationDatasource } from "../../domain/datasources/AbstractUserLocationDatasoruce";
...
export class UserLocationDatasourceImpl
  implements AbstractUserLocationDatasource
{
  ...
  async updateLocation(
    ...
    const row = await this.controller.updateLocation(
      userId,
      latitude,
      longitude
    );
  ...
  async listNearbyUsers(
    ...
    return res.profiles.map((row: any) =>
      toDomainUserLocation(row, row.updated_at)
    );
  }
